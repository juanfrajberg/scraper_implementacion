...
def tweet_to_dict(tweet):
    ...
    return {
        ...
        "conversation_id": (
            str(tweet.conversationId) if tweet.conversationId is not None else None
        ),
        "in_reply_to": (
            str(tweet.inReplyToTweetId) if tweet.inReplyToTweetId is not None else None
        ),
        "in_reply_to_user": (
            tweet.inReplyToUser.username if tweet.inReplyToUser is not None else None
        ),
        "mentioned_users": [user.username for user in tweet.mentionedUsers],
...
def save_jsonl(path: Path, data: list[dict], key: str):
    ...
    print(f"Agregados {len(new_items)} registros a {path}")
...
async def collect_tweets(
    ...
    tweet_data = [tweet_to_dict(tweet) for tweet in tweets]
    ...
    print(f"Conversaciones únicas: {len(conversations)}")

    # ---------------------------------------------------------
    # 3. Conversaciones ya existentes
    # ---------------------------------------------------------

    existing_conversations = get_existing_conversation_ids(THREADS_FILE)

    print(f"Conversaciones ya guardadas: {len(existing_conversations)}")
    ...
    for conversation_id, root_tweet in conversations.items():
        if conversation_id in existing_conversations:
            print(f"Conversación {conversation_id} ya existe. Se omite.")
            continue

        print(f"Reconstruyendo conversación {conversation_id}...")
        ...
        thread_ids = {str(tweet.id) for tweet in thread}
        ...
        thread_data = {
            ...
            "tweets": [tweet_to_dict(tweet) for tweet in thread],
        }

        threads_data.append(thread_data)

        print(f"Thread {conversation_id}: {len(thread)} tweets")
