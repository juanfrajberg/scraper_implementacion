        ...
        *,
        ...
        with self.connect() as database:
            rows = database.execute(
                f"""
                SELECT
                    MIN(t.tweet_id) AS tweet_id,
                    COALESCE(NULLIF(t.conversation_id, ''), t.tweet_id) AS conversation_id,
                    t.created_at,
                    MAX(COALESCE(t.reply_count, 0)) AS reply_count
                FROM tweets t
                JOIN captures c ON c.tweet_id = t.tweet_id
                JOIN jobs j ON j.job_id = c.job_id
                WHERE c.capture_kind = 'search'
                  AND COALESCE(t.reply_count, 0) >= ?
                  AND j.corpus_layer = ?
                  {family_filter}
                GROUP BY COALESCE(NULLIF(t.conversation_id, ''), t.tweet_id)
                ORDER BY reply_count DESC, t.created_at, conversation_id
                LIMIT ?
                """,
                parameters,
            ).fetchall()
